...
/***/ function(module, exports, __webpack_require__) {
	...
	var App = React.createClass({
		...
		getInitialState: function () {
			return {
				gold: 0,
				locations: [{ name: 'Farm', min: 10, max: 20 }, { name: 'Cave', min: 5, max: 10 }, { name: 'House', min: 2, max: 5 }, { name: 'Casino', min: -50, max: 50 }],
				history: []
			};
		},
		render: function () {
			...
			var goldSources = this.state.locations.map(function (location) {
				return React.createElement(GoldSource, { key: location.name, name: location.name, min: location.min, max: location.max, findGold: _that.findGold });
			});
			return React.createElement(
				'div',
				{ className: 'app' },
				React.createElement(GoldDisplay, { gold: this.state.gold }),
				React.createElement(
					'div',
					{ className: 'location-container' },
					goldSources
				),
				React.createElement(SearchHistory, { history: this.state.history })
			);
		},
	...
	var GoldSource = React.createClass({
		displayName: 'GoldSource',

		render: function () {
			return React.createElement(
				'div',
				{ className: 'gold-source' },
				React.createElement(
					'h3',
					null,
					this.props.name
				...
				React.createElement(
					'p',
					null,
					this.props.name !== 'Casino' ? `Earns ${ this.props.min }-${ this.props.max }` : `Earns/Takes 0-${ this.props.max }`
				),
				React.createElement(
					'button',
					{ onClick: this.goldSearch },
					'Find Gold!'
				)
			);
		},
	...
	var SearchHistory = React.createClass({
		...
		render: function () {
			var searchEvents = this.props.history.map(function (event, index) {
				return React.createElement(SearchEvent, { key: index, event: event });
			});
			return React.createElement(
				'div',
				null,
	...
	var SearchEvent = React.createClass({
		displayName: 'SearchEvent',

		render: function () {
			return React.createElement(
				'li',
				{ className: this.props.event.verb },
				this.props.event.resultDescription
			);
		}
	});
